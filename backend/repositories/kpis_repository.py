...
def _s(k: KpiClienteMkt) -> dict:
    ...
    return {
        ...
        "kpi": k.kpi,
        "activo": k.activo,
        "valor_actual": float(k.valor_actual),
...
def obtener_activos(db: Session, marca_id: UUID) -> list:
    ...
    rows = (
        db.query(KpiClienteMkt)
        .filter(
            KpiClienteMkt.marca_id == marca_id,
            KpiClienteMkt.activo == True,
        )
        .all()
    )
