...
def _s(m: MensajeComunidadMkt) -> dict:
    ...
    return {
        ...
        "autor_id_externo": m.autor_id_externo,
        ...
        "escalado": m.escalado,
        "motivo_escalado": m.motivo_escalado,
        "es_lead": m.es_lead,
...
def listar(db: Session, marca_id: UUID) -> list:
    ...
    rows = db.query(MensajeComunidadMkt).filter(MensajeComunidadMkt.marca_id == marca_id).order_by(MensajeComunidadMkt.created_at.desc()).all()
...
def listar_respondidos(db: Session, marca_id: UUID) -> list:
    ...
    rows = (
        ...
        .order_by(MensajeComunidadMkt.respondido_at.desc())
        .all()
    )
    return [_s(r) for r in rows]
...
def listar_leads(db: Session, marca_id: UUID) -> list:
    ...
    rows = (
        ...
        .order_by(MensajeComunidadMkt.created_at.desc())
        .all()
    )
    return [_s(r) for r in rows]
