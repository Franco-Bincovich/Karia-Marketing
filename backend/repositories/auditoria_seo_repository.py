...
def listar(db: Session, marca_id: UUID) -> list:
    ...
    rows = db.query(AuditoriaSeoMkt).filter(AuditoriaSeoMkt.marca_id == marca_id).order_by(AuditoriaSeoMkt.created_at.desc()).all()
...
def marcar_implementado(db: Session, item_id: UUID, marca_id: UUID) -> dict:
    ...
    obj = (
        db.query(AuditoriaSeoMkt)
        .filter(
            AuditoriaSeoMkt.id == item_id,
...
def listar_pendientes(db: Session, marca_id: UUID) -> list:
    ...
    rows = (
        ...
        .filter(
            AuditoriaSeoMkt.marca_id == marca_id,
            AuditoriaSeoMkt.implementado == False,
        )
        .all()
    )
