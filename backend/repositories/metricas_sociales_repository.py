...
def _s(m: MetricasSocialesMkt) -> dict:
    ...
    return {
        ...
        "red_social": m.red_social, "fecha": m.fecha.isoformat(),
        "alcance": m.alcance, "impresiones": m.impresiones,
        "engagement": m.engagement, "engagement_rate": float(m.engagement_rate),
        "nuevos_seguidores": m.nuevos_seguidores, "clicks": m.clicks,
...
def guardar_metricas(db: Session, data: dict) -> dict:
    ...
    existing = db.query(MetricasSocialesMkt).filter(
...
def promedio_por_canal(db: Session, marca_id: UUID) -> dict:
    ...
    ).filter(MetricasSocialesMkt.marca_id == marca_id).group_by(
        MetricasSocialesMkt.red_social
    ).all()
...
def total_periodo(db: Session, marca_id: UUID, fecha_inicio: date, fecha_fin: date) -> dict:
    ...
    return {
        "alcance": int(row[0]), "impresiones": int(row[1]),
        "engagement": int(row[2]), "clicks": int(row[3]),
