...
def _s(c: ContenidoMkt) -> dict:
    return {
        ...
        "copy_b": c.copy_b,
        "copy_c": c.copy_c,
        "hashtags_a": c.hashtags_a,
        "hashtags_b": c.hashtags_b,
        "hashtags_c": c.hashtags_c,
        "cta_a": c.cta_a,
        "cta_b": c.cta_b,
        "cta_c": c.cta_c,
...
def _sv(v: VersionesContenidoMkt) -> dict:
    return {
        "id": str(v.id),
        "contenido_id": str(v.contenido_id),
        "version": v.version,
        "copy_a": v.copy_a,
        "copy_b": v.copy_b,
...
def contar_mes_actual(db: Session, marca_id: UUID) -> int:
    ...
    return (
        db.query(func.count(ContenidoMkt.id))
        .filter(
...
def obtener(db: Session, contenido_id: UUID, marca_id: UUID) -> Optional[ContenidoMkt]:
    ...
    return db.query(ContenidoMkt).filter(ContenidoMkt.id == contenido_id, ContenidoMkt.marca_id == marca_id).first()
...
def guardar_version(db: Session, contenido_id: UUID, version_data: dict) -> dict:
    ...
    ultimo = db.query(func.max(VersionesContenidoMkt.version)).filter(VersionesContenidoMkt.contenido_id == contenido_id).scalar() or 0
...
def listar_versiones(db: Session, contenido_id: UUID) -> list[dict]:
    ...
    rows = (
        db.query(VersionesContenidoMkt).filter(VersionesContenidoMkt.contenido_id == contenido_id).order_by(VersionesContenidoMkt.version.asc()).all()
    )
