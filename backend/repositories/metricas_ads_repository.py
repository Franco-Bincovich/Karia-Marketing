...
def listar_por_campana(db: Session, campana_id: UUID) -> list:
    rows = db.query(MetricasAdsMkt).filter(MetricasAdsMkt.campana_id == campana_id).order_by(MetricasAdsMkt.fecha.desc()).all()
...
def gasto_diario_total(db: Session, marca_id: UUID, fecha: date) -> float:
    ...
    result = (
        db.query(func.coalesce(func.sum(MetricasAdsMkt.gasto), Decimal(0)))
        .filter(
