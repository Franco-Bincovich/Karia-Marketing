    ...
    for kpi in kpis:
        ...
        if objetivo and objetivo > 0 and valor < objetivo * (1 - ANOMALIA_UMBRAL):
            anomalia = {
                ...
                "diferencia_pct": round((1 - valor / objetivo) * 100, 1),
            }
...
def obtener_dashboard(db: Session, marca_id: UUID) -> dict:
    ...
    for kpi in kpis:
        ...
        progreso.append({"kpi": kpi["kpi"], "actual": kpi["valor_actual"], "objetivo": obj, "progreso_pct": pct})

    return {
        "kpis": progreso,
        "metricas_30d": totales,
        "promedios_canal": promedios,
        "periodo": f"{inicio.isoformat()} a {hoy.isoformat()}",
    }
