...
def buscar_menciones(db: Session, marca_id: UUID) -> dict:
    ...
    umbral = config.get("umbral_urgencia", 1000)
    items = []
    ...
    for item in items:
        if item["urgente"]:
            alertas_repo.crear(db, {
                "marca_id": marca_id, "tipo": "mencion_negativa",
...
def listar_alertas(db: Session, marca_id: UUID) -> list[dict]:
    ...
    return alertas_repo.listar_por_tipo(db, marca_id, "mencion_negativa") + \
           alertas_repo.listar_por_tipo(db, marca_id, "crisis_detectada")
...
def _detectar_crisis(db: Session, marca_id: UUID) -> bool:
    ...
    neg_48 = sum(1 for m in menciones if m["sentimiento"] == "negativo" and m.get("created_at") and hace_48h.isoformat() < m["created_at"] <= hace_24h.isoformat())

    if neg_48 > 0 and neg_24 > neg_48 * 1.5:
        alertas_repo.crear(db, {
            "marca_id": marca_id, "tipo": "crisis_detectada",
