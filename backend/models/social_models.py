...
class CalendarioEditorialMkt(Base):
    ...
    contenido_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("contenido_mkt.id", ondelete="SET NULL"))
...
class PublicacionesMkt(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    marca_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("marcas_mkt.id", ondelete="CASCADE"), nullable=False)
    calendario_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("calendario_editorial_mkt.id", ondelete="SET NULL"))
    contenido_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("contenido_mkt.id", ondelete="SET NULL"))
    red_social: Mapped[str] = mapped_column(Text, nullable=False)
    post_id_externo: Mapped[str | None] = mapped_column(Text)
    url_publicacion: Mapped[str | None] = mapped_column(Text)
    copy_publicado: Mapped[str | None] = mapped_column(Text)
...
class CuentasSocialesMkt(Base):
    ...
    account_id_externo: Mapped[str | None] = mapped_column(Text)
