...
const FORMAT_SIZE = { post: "1024x1024", carrusel: "1024x1024", reel: "1024x1792", story: "1024x1792" };
...
function PreviewPost({ c, formato }) {
  ...
  return (
    <div style={{ border: "1px solid var(--border)", borderRadius: 12, overflow: "hidden", maxWidth: isVertical ? 270 : 320 }}>
      {c.imagen_url && (
        <img src={c.imagen_url} alt="" style={{ width: "100%", aspectRatio: isVertical ? "9/16" : "1/1", objectFit: "cover", display: "block" }}
...
export default function Contenido() {
  ...
  const { user } = useAuth();
  const [form, setForm] = useState({ red_social: "instagram", formato: "post", objetivo: "", tono: "profesional", tema: "", num_slides: 5 });
  ...
  async function generar(modo = "copilot") {
    if (!form.tema) return;
    setLoading(true); setError(""); setCarruselSlides(null);
    try {
  ...
  async function generarCarrusel() {
    ...
    try {
      const { data } = await post(ENDPOINTS.IMAGENES_CARRUSEL(resultado.id), { num_slides: form.num_slides, estilo: "vivid" });
      setCarruselSlides(data.slides || []);
      setPubMsg(`Carrusel generado: ${data.num_slides} slides`);
    } catch (e) { setPubMsg(e.response?.data?.message || "Error generando carrusel"); }
    finally { setActionId(null); }
  }
  ...
  return (
    <Layout title="Generar Contenido">
      ...
      <div style={card}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: isCarrusel ? "1fr 1fr 1fr 1fr" : "1fr 1fr 1fr", gap: 12, marginBottom: 12 }}>
          <div><label className="field-label-sm">Red Social</label><select style={select} value={form.red_social} onChange={setField("red_social")}><option>instagram</option><option>facebook</option><option>linkedin</option><option>tiktok</option><option>twitter</option></select></div>
          <div><label className="field-label-sm">Formato</label><select style={select} value={form.formato} onChange={setField("formato")}><option>post</option><option>reel</option><option>story</option><option>carrusel</option></select></div>
          <div><label className="field-label-sm">Tono</label><select style={select} value={form.tono} onChange={setField("tono")}><option>profesional</option><option>cercano</option><option>inspirador</option><option>humoristico</option></select></div>
          {isCarrusel && (
            <div><label className="field-label-sm">Slides</label><select style={select} value={form.num_slides} onChange={e => setForm({ ...form, num_slides: parseInt(e.target.value) })}>{[2,3,4,5,6,7,8,9,10].map(n => <option key={n} value={n}>{n} slides</option>)}</select></div>
          )}
        </div>
        ...
        <div style={{ fontSize: 11, color: "var(--text-muted)", marginBottom: 12 }}>
          Tamaño de imagen: <strong>{FORMAT_SIZE[form.formato] || "1024x1024"}</strong> ({form.formato === "story" || form.formato === "reel" ? "vertical" : "cuadrado"})
        </div>
      ...
      {resultado && resultado.estado === "aprobado" && (
        <div style={card}>
          <h3 style={{ fontSize: 15, fontWeight: 700, marginBottom: 12, color: "var(--text)" }}>Variante aprobada</h3>
          <div style={{ display: "grid", gridTemplateColumns: resultado.imagen_url ? "auto 1fr" : "1fr", gap: 20 }}>
            {resultado.imagen_url && <PreviewPost c={resultado} formato={resultado.formato} />}
            <div>
              <p style={{ fontSize: 14, color: "var(--text)", lineHeight: 1.6, whiteSpace: "pre-wrap", marginBottom: 12 }}>
                {resultado[`copy_${resultado.variante_seleccionada || "a"}`] || resultado.copy_a}
              </p>
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                {!resultado.imagen_url && !isCarrusel && (
                  <button style={{ ...btnSmall, borderColor: "#EC4899", color: "#EC4899" }} onClick={generarImagenParaCopy} disabled={actionId === "gen_img"}>
                    {actionId === "gen_img" ? "Generando imagen..." : "Generar imagen para este copy"}
                  </button>
                )}
                {!resultado.imagen_url && isCarrusel && (
                  <button style={{ ...btnSmall, borderColor: "#EC4899", color: "#EC4899" }} onClick={generarCarrusel} disabled={actionId === "gen_carrusel"}>
                    {actionId === "gen_carrusel" ? "Generando carrusel..." : `Generar carrusel (${form.num_slides} slides)`}
                  </button>
                )}
                <button style={{ ...btnSmall, borderColor: "var(--success)", color: "var(--success-text)" }} onClick={() => publicarAhora(resultado)}>Publicar ahora</button>
                <button style={btnSmall} onClick={() => { setResultado(null); loadData(); }}>Cerrar</button>
              </div>
            </div>
          </div>
          {carruselSlides && carruselSlides.length > 0 && (
            <div style={{ marginTop: 16 }}>
              <h4 style={{ fontSize: 13, fontWeight: 700, marginBottom: 8, color: "var(--text)" }}>Slides del carrusel</h4>
              <div style={{ display: "flex", gap: 12, overflowX: "auto", paddingBottom: 8 }}>
                {carruselSlides.map(s => (
                  <div key={s.id} style={{ minWidth: 180, border: "1px solid var(--border)", borderRadius: 10, overflow: "hidden", flexShrink: 0 }}>
                    {s.imagen_url && <img src={s.imagen_url} alt="" style={{ width: "100%", height: 180, objectFit: "cover" }} onError={e => { e.target.style.display = "none"; }} />}
                    <div style={{ padding: "8px 10px" }}>
                      <div style={{ fontSize: 10, color: "var(--text-muted)", marginBottom: 2 }}>Slide {s.orden}</div>
          ...
          <div key={c.id} style={{ padding: "12px 0", borderBottom: "1px solid var(--border-subtle)" }}>
            ...
            {c.imagen_url && (
              <div style={{ display: "flex", gap: 10, marginBottom: 6 }}>
                <img src={c.imagen_url} alt="" style={{ height: 56, width: c.formato === "story" || c.formato === "reel" ? 32 : 56, borderRadius: 6, objectFit: "cover" }} onError={e => { e.target.style.display = "none"; }} />
                <p style={{ fontSize: 12, color: "var(--text-muted)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", maxWidth: 400, alignSelf: "center" }}>
                  {c[`copy_${c.variante_seleccionada || "a"}`] || c.copy_a || ""}
                </p>
              </div>
            )}
