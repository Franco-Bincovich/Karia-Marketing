      ...
      <div style={card}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr auto", gap: 12, alignItems: "end" }}>
          ...
          <div>
            ...
            <select style={{ ...inputStyle, appearance: "auto" }} value={form.plataforma} onChange={e => setForm({ ...form, plataforma: e.target.value })}>
              <option>meta</option><option>google</option>
            </select>
          </div>
          <div>
            <label style={{ fontSize: 12, color: "var(--text-muted)" }}>Presupuesto/día ($)</label>
            <input style={inputStyle} type="number" value={form.presupuesto_diario} onChange={e => setForm({ ...form, presupuesto_diario: e.target.value })} />
          </div>
          <div>
            <label style={{ fontSize: 12, color: "var(--text-muted)" }}>Objetivo</label>
            <input style={inputStyle} value={form.objetivo} onChange={e => setForm({ ...form, objetivo: e.target.value })} />
          </div>
      ...
      <div style={card}>
        ...
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead>
              <tr>
                <th style={th}>Nombre</th><th style={th}>Plataforma</th>
                <th style={th}>Presupuesto/día</th><th style={th}>Estado</th><th style={th}>Acciones</th>
              </tr>
            </thead>
            <tbody>
              {campanas.length === 0 && (
                <tr><td colSpan={5} style={{ ...td, textAlign: "center", color: "var(--text-muted)" }}>No hay campañas</td></tr>
              )}
              {campanas.map(c => (
                <tr key={c.id}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 16 }}>
        {[["CPA Máximo", umbrales.cpa_maximo], ["ROAS Mínimo", umbrales.roas_minimo], ["Gasto Diario Máx", umbrales.gasto_diario_maximo]].map(([label, val]) => (
          <div key={label} style={card}>
            ...
            <div style={{ fontSize: 26, fontWeight: 800, color: "var(--text)" }}>${val || 0}</div>
          </div>
        ))}
      </div>
    </Layout>
