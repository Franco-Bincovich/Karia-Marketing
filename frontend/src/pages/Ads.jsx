      ...
      <Layout title="Campañas Ads">
        <div style={{ ...card, padding: 60, textAlign: "center" }}>
          ...
          <div style={{ display: "flex", justifyContent: "center", gap: 8, marginBottom: 12 }}>
            ...
            <span style={{ background: "var(--purple-bg)", color: "var(--purple-text)", padding: "3px 10px", borderRadius: 6, fontSize: 11, fontWeight: 700, alignSelf: "center" }}>V2</span>
          </div>
...
function AdsPanel() {
  ...
  return (
    <Layout title="Campañas Ads">
      <div style={card}>
        <h3 style={{ fontSize: 15, fontWeight: 700, marginBottom: 16, color: "var(--text)" }}>Nueva Campaña</h3>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr auto", gap: 12, alignItems: "end" }}>
          <div><label style={{ fontSize: 12, color: "var(--text-muted)" }}>Nombre</label><input style={inputStyle} value={form.nombre} onChange={e => setForm({ ...form, nombre: e.target.value })} /></div>
          <div><label style={{ fontSize: 12, color: "var(--text-muted)" }}>Plataforma</label><select style={{ ...inputStyle, appearance: "auto" }} value={form.plataforma} onChange={e => setForm({ ...form, plataforma: e.target.value })}><option>meta</option><option>google</option></select></div>
          <div><label style={{ fontSize: 12, color: "var(--text-muted)" }}>Presupuesto/día ($)</label><input style={inputStyle} type="number" value={form.presupuesto_diario} onChange={e => setForm({ ...form, presupuesto_diario: e.target.value })} /></div>
          <div><label style={{ fontSize: 12, color: "var(--text-muted)" }}>Objetivo</label><input style={inputStyle} value={form.objetivo} onChange={e => setForm({ ...form, objetivo: e.target.value })} /></div>
          <button style={btn} onClick={crear} disabled={loading}>Crear</button>
        </div>
      </div>

      <div style={card}>
        <h3 style={{ fontSize: 15, fontWeight: 700, marginBottom: 12, color: "var(--text)" }}>Campañas ({campanas.length})</h3>
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            ...
            <tbody>
              {campanas.length === 0 && <tr><td colSpan={5} style={{ ...td, textAlign: "center", color: "var(--text-muted)" }}>No hay campañas</td></tr>}
              {campanas.map(c => (
                <tr key={c.id}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 16 }}>
        {[["CPA Máximo", umbrales.cpa_maximo], ["ROAS Mínimo", umbrales.roas_minimo], ["Gasto Diario Máx", umbrales.gasto_diario_maximo]].map(([label, val]) => (
          <div key={label} style={card}>
            ...
            <div style={{ fontSize: 26, fontWeight: 800, color: "var(--text)" }}>{val != null ? `$${val}` : "--"}</div>
            {val == null && <div style={{ fontSize: 11, color: "var(--text-muted)", marginTop: 2 }}>Sin configurar</div>}
          </div>
        ))}
      </div>
    </Layout>
