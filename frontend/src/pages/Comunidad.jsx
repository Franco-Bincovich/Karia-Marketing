    ...
    <Layout title="Comunidad">
      ...
      {tab === "pendientes" && (
        <div>
          {pendientes.length === 0 ? (
            <div style={card}>
              <EmptyState icon="💬" title="Bandeja vacía" description="No hay mensajes pendientes de respuesta" />
            </div>
          ) : pendientes.map(m => (
            <div key={m.id} style={card}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
                <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                  ...
                  <span style={{ fontWeight: 600, fontSize: 13, color: "var(--text)" }}>{m.autor_username || "Anónimo"}</span>
                  <span style={{ fontSize: 11, color: "var(--text-muted)" }}>{m.tipo}</span>
                </div>
                <div style={{ display: "flex", gap: 6 }}>
                  <span style={sentBadge(m.sentimiento)}>{m.sentimiento}</span>
                  <span style={{ background: "var(--blue-bg)", color: "var(--blue-text)", padding: "2px 8px", borderRadius: 6, fontSize: 11, fontWeight: 600 }}>{m.clasificacion}</span>
                </div>
              </div>
              <p style={{ fontSize: 14, color: "var(--text)", marginBottom: 10, lineHeight: 1.5 }}>{m.contenido}</p>
              {m.respuesta_sugerida && (
                <div style={{ background: "var(--primary-light)", borderRadius: 8, padding: "8px 12px", marginBottom: 10, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <p style={{ fontSize: 13, color: "var(--primary)", fontStyle: "italic", flex: 1 }}>IA sugiere: {m.respuesta_sugerida}</p>
                  <button style={{ ...btnSmall, borderColor: "var(--primary)", color: "var(--primary)" }} onClick={() => usarSugerencia(m.id, m.respuesta_sugerida)}>Usar</button>
                </div>
              )}
      ...
      {tab === "historial" && (
        <div style={{ ...card, overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 600 }}>
            <thead>
              <tr>
                <th style={th}>Red</th><th style={th}>Usuario</th><th style={th}>Mensaje</th>
                <th style={th}>Respuesta</th><th style={th}>Fecha</th>
              </tr>
            </thead>
            <tbody>
              {historial.length === 0 && (
                <tr><td colSpan={5} style={{ ...td, textAlign: "center", color: "var(--text-muted)" }}>Sin historial</td></tr>
              )}
              {historial.map(m => (
                <tr key={m.id}>
                  <td style={td}>{redIcon[m.red_social] || "🌐"} {m.red_social}</td>
                  <td style={{ ...td, fontWeight: 500, color: "var(--text)" }}>{m.autor_username || "—"}</td>
                  <td style={{ ...td, maxWidth: 200, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{m.contenido}</td>
                  <td style={{ ...td, maxWidth: 200, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", color: "var(--success-text)" }}>{m.respuesta || "—"}</td>
                  <td style={{ ...td, fontSize: 11, color: "var(--text-muted)" }}>{m.respondido_at ? new Date(m.respondido_at).toLocaleDateString("es-AR") : "—"}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {tab === "leads" && (
        <div style={card}>
          {leads.length === 0 ? (
            <EmptyState icon="🎯" title="Sin leads detectados" description="Los leads aparecen cuando detectamos interés de compra en comentarios" />
          ) : leads.map(l => (
            <div key={l.id} style={{ padding: "10px 0", borderBottom: "1px solid var(--border-subtle)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div>
                <span style={{ fontSize: 13, fontWeight: 600, color: "var(--text)" }}>{l.autor_username || "Anónimo"}</span>
                <span style={{ fontSize: 11, color: "var(--text-muted)", marginLeft: 8 }}>{l.red_social}</span>
                <p style={{ fontSize: 12, color: "var(--text-secondary)", marginTop: 2 }}>{l.contenido?.slice(0, 100)}</p>
              </div>
              <span style={{ background: "var(--success-bg)", color: "var(--success-text)", padding: "2px 8px", borderRadius: 6, fontSize: 11, fontWeight: 600 }}>Lead</span>
            </div>
          ))}
        </div>
      )}
