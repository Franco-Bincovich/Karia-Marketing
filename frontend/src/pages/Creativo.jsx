...
export default function Creativo() {
  ...
  useEffect(() => {
    ...
    get(ENDPOINTS.IMAGENES_BIBLIOTECA)
      .then((r) => setBiblioteca(r.data.data || []))
      .catch(() => {});
  }, []);
  ...
  async function generar() {
    ...
    try {
      ...
      setError(e.response?.data?.message || "Error al generar imagen");
    } finally {
      setLoading(false);
    }
  }
    ...
    <Layout title="Creativo IA">
      ...
      {tab === "ia" && (
        <>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
            <div style={card}>
              ...
              <label style={{ fontSize: 12, color: "var(--text-muted)" }}>
                {form.tipo === "placa" ? "Texto principal de la placa" : "Descripción de la imagen"}
              </label>
              <textarea
                ...
                placeholder={
                  form.tipo === "placa"
                    ? "Escribí el texto principal que querés mostrar en la placa..."
                    : "Describí la imagen que querés generar..."
                }
              />
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                <div>
                  <label style={{ fontSize: 12, color: "var(--text-muted)" }}>Formato</label>
                  <select style={selectStyle} value={form.formato} onChange={set("formato")}>
                    {FORMATOS.map((f) => (
              ...
              >
                <input
                  type="checkbox"
                  checked={form.usar_perfil_marca}
                  onChange={(e) => setForm({ ...form, usar_perfil_marca: e.target.checked })}
                />
          ...
          <div style={card}>
            <h3 style={{ fontSize: 15, fontWeight: 700, marginBottom: 16, color: "var(--text)" }}>
              Galería IA ({galeria.length})
            </h3>
            {galeria.length === 0 ? (
              <EmptyState
                icon="🖼️"
                title="Sin imágenes"
                description="Generá tu primera imagen con IA"
              />
            ) : (
                ...
                {galeria.map((img) => (
                  ...
                  >
                    <img
                      ...
                      onError={(e) => {
                        e.target.style.display = "none";
                        e.target.parentNode.style.background = "var(--surface-2)";
                        e.target.parentNode.style.height = "140px";
                      }}
                    />
      ...
      {tab === "biblioteca" && (
        <>
          ...
          <div style={card}>
            <h3 style={{ fontSize: 15, fontWeight: 700, marginBottom: 16, color: "var(--text)" }}>
              Imágenes subidas ({biblioteca.length})
            </h3>
            {biblioteca.length === 0 ? (
