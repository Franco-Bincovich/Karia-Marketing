...
export default function AgentesIA() {
  ...
  async function toggleActivo(agente) {
    try {
      const { data } = await patch(ENDPOINTS.AGENTE_CONFIG(agente.nombre), {
  ...
  async function guardarPrompt() {
    ...
    setSaving(true);
    try {
      const { data } = await patch(ENDPOINTS.AGENTE_CONFIG(configOpen.nombre), {
        system_prompt: promptEdit,
      });
  ...
  function ejecutar(agente) {
    if (agente.nombre === "creativo") {
      navigate("/creativo");
      return;
    }
    if (agente.nombre === "comunidad") {
      navigate("/comunidad");
      return;
    }
    if (agente.nombre === "estrategia") {
      navigate("/estrategia");
      return;
    }
  ...
  async function ejecutarRemoto(agente) {
    ...
    try {
      ...
      setResultado({
        ...
        resultado: e.response?.data?.message || "Error al ejecutar",
      });
    } finally {
      setExecuting(null);
    }
      ...
      >
        {agentes.map((a) => {
          ...
          return (
            <div
              key={a.nombre}
              style={{ ...card, opacity: isBlocked ? 0.55 : 1, position: "relative" }}
            >
                ...
                {!isBlocked && a.activo && (
                  <button
                    ...
                    style={{
                      ...
                      fontWeight: 600,
                      border: "none",
                      cursor: a.autopilot_disponible ? "pointer" : "default",
                    ...
                    title={
                      a.autopilot_disponible
                        ? "Click para cambiar modo"
                        : "Autopilot solo disponible en Premium"
                    }
                  >
              ...
              {!isBlocked && (
                <div style={{ display: "flex", gap: 6 }}>
                  <button style={btnSmall} onClick={() => abrirConfig(a)}>
                    Configurar
                  </button>
                  {a.activo && (
                    <button
        ...
        <>
          ...
          <div className="drawer">
            <div className="drawer-header">
              ...
              <button onClick={() => setConfigOpen(null)} className="modal-close">
                ×
              </button>
            </div>

            <div style={{ marginBottom: 20 }}>
              <label className="field-label-sm" style={{ marginBottom: 6 }}>
                System Prompt
              </label>
              <textarea
                style={{
