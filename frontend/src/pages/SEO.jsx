...
export default function SEO() {
  ...
  useEffect(() => {
    get(ENDPOINTS.SEO_KEYWORDS)
      .then((r) => setKeywords(r.data.data || []))
      .catch(() => {});
    get(ENDPOINTS.SEO_AUDITORIA)
      .then((r) => setAuditoria(r.data.data || []))
      .catch(() => {});
    get(ENDPOINTS.SEO_BRIEFS)
    ...
    <Layout title="SEO">
      <div style={card}>
        ...
        <div style={{ display: "flex", gap: 12 }}>
          ...
          />
          <button style={btn} onClick={investigar} disabled={loading === "kw"}>
            {loading === "kw" ? "..." : "Investigar"}
          </button>
        </div>
      </div>
      ...
      <div style={card}>
        <h3 style={{ fontSize: 15, fontWeight: 700, marginBottom: 12, color: "var(--text)" }}>
          Keywords Monitoreadas ({keywords.length})
        </h3>
        {keywords.length === 0 ? (
          <EmptyState
            icon="🔍"
            title="Sin keywords"
            description="Investigá keywords para empezar a monitorearlas"
          />
        ) : (
                ...
                {keywords.slice(0, 20).map((k) => (
                  <tr key={k.id}>
                    ...
                    <td style={td}>
                      ...
                      {k.posicion_anterior &&
                      k.posicion_actual &&
                      k.posicion_actual < k.posicion_anterior ? (
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
        <div style={card}>
          ...
          <div style={{ display: "flex", gap: 12, marginBottom: 12 }}>
            <input
              style={{ ...input, flex: 1 }}
              placeholder="URL a auditar"
              value={url}
              onChange={(e) => setUrl(e.target.value)}
            />
            <button style={btn} onClick={auditar} disabled={loading === "audit"}>
              Auditar
            </button>
          </div>
          {auditoria.length === 0 ? (
            <EmptyState
              ...
              description="Ingresá una URL para auditar"
            />
          ) : (
        ...
        <div style={card}>
          ...
          <div style={{ display: "flex", gap: 12, marginBottom: 12 }}>
            ...
            />
            <button style={btn} onClick={generarBrief} disabled={loading === "brief"}>
              Generar
            </button>
          </div>
          {briefs.length === 0 ? (
            <EmptyState
              icon="📝"
              title="Sin briefs"
              description="Generá un brief para optimizar contenido"
            />
          ) : (
