...
export default function Onboarding() {
  ...
  useEffect(() => {
    get(ENDPOINTS.ONBOARDING_ESTADO).then(r => {
      ...
      const first = (r.data.preguntas || []).findIndex(p => p.obligatoria && !resp[String(p.id)]);
  ...
  const preguntasFiltradas = preguntas.map((p, i) => ({ ...p, _idx: i })).filter(p => {
    ...
    <Layout title="Configuración de Marca">
      ...
      <div style={card}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
          ...
          <span style={{ fontSize: 24, fontWeight: 800, color: obligCompletas ? "var(--success-text)" : "var(--primary)" }}>{comp}%</span>
        </div>
        <div className="progress-track">
          <div className={`progress-fill ${obligCompletas ? "success" : ""}`} style={{ width: `${comp}%` }} />
        </div>
      ...
      {!obligCompletas && (
        <div style={{ ...card, borderColor: "var(--warning)", background: "var(--warning-bg)" }}>
          <div style={{ fontSize: 13, color: "var(--warning-text)", lineHeight: 1.5 }}>
            <strong>Recomendación:</strong> Completá las {obligTotal - obligResp} preguntas obligatorias restantes
            para que los agentes de IA trabajen mejor con tu marca.
            Con la información actual, el Copilot puede funcionar pero los resultados serán más genéricos.
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              ...
              {preguntaActual.obligatoria ? (
                ...
                <span style={{ fontSize: 10, color: "var(--text-muted)", background: "var(--surface-2)", padding: "1px 6px", borderRadius: 4, fontWeight: 600 }}>Opcional</span>
              )}
            </div>
      ...
      <div style={card}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
          ...
          <div style={{ display: "flex", gap: 4 }}>
            {[{ k: "todas", l: "Todas" }, { k: "obligatorias", l: "Obligatorias" }, { k: "opcionales", l: "Opcionales" }].map(f => (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
          {preguntasFiltradas.map(p => {
            ...
            return (
              <button key={p.id} onClick={() => setPasoActivo(p._idx)} style={{
