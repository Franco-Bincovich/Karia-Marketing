...
const input   = { width: "100%", padding: "10px 12px", border: "1.5px solid var(--border)", borderRadius: 9, fontSize: 14, outline: "none", marginBottom: 12, boxSizing: "border-box", background: "var(--surface)", color: "var(--text)" };
...
export default function Prospeccion() {
  ...
  return (
    <Layout title="Prospección IA">
      <div style={card}>
        <h3 style={{ fontSize: 15, fontWeight: 700, marginBottom: 16, color: "var(--text)" }}>Buscar Prospectos</h3>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr auto auto", gap: 12, alignItems: "end" }}>
          <div>
            <label style={{ fontSize: 12, color: "var(--text-muted)" }}>Rubro</label>
            <input style={input} value={rubro} onChange={e => setRubro(e.target.value)} placeholder="Ej: tecnología" />
          </div>
          <div>
            <label style={{ fontSize: 12, color: "var(--text-muted)" }}>Ubicación</label>
            <input style={input} value={ubicacion} onChange={e => setUbicacion(e.target.value)} placeholder="Ej: Buenos Aires" />
          </div>
          <div>
            <label style={{ fontSize: 12, color: "var(--text-muted)" }}>Cantidad</label>
            <input style={{ ...input, width: 80 }} type="number" value={cantidad} onChange={e => setCantidad(e.target.value)} />
          </div>
      ...
      {resultados.length > 0 && (
        <div style={card}>
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 12 }}>
            <h3 style={{ fontSize: 15, fontWeight: 700, color: "var(--text)" }}>Resultados ({resultados.length})</h3>
          ...
          <div style={{ overflowX: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              <thead>
                <tr>
                  <th style={th}></th><th style={th}>Nombre</th><th style={th}>Empresa</th>
                  <th style={th}>Email</th><th style={th}>Cargo</th><th style={th}>Confianza</th>
                </tr>
              </thead>
              <tbody>
                {resultados.map((c, i) => (
                  <tr key={i} style={{ background: seleccionados.has(i) ? "var(--primary-light)" : "transparent" }}>
                    ...
                    <td style={td}>
                      ...
                      }}>{Math.round((c.confianza || 0) * 100)}%</span>
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      <div style={card}>
        <h3 style={{ fontSize: 15, fontWeight: 700, marginBottom: 12, color: "var(--text)" }}>Contactos Guardados ({contactos.length})</h3>
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead>
              <tr>
                <th style={th}>Nombre</th><th style={th}>Empresa</th>
                <th style={th}>Email</th><th style={th}>Score</th><th style={th}>Estado</th>
              </tr>
            </thead>
            <tbody>
              {contactos.length === 0 && (
                <tr><td colSpan={5} style={{ ...td, textAlign: "center", color: "var(--text-muted)" }}>Sin contactos guardados</td></tr>
              )}
              {contactos.map(c => (
                <tr key={c.id}>
                  <td style={{ ...td, color: "var(--text)", fontWeight: 500 }}>{c.nombre}</td>
                  ...
                  <td style={td}>
                    <span style={{ background: "var(--blue-bg)", color: "var(--blue-text)", padding: "2px 8px", borderRadius: 6, fontSize: 11, fontWeight: 600 }}>{c.estado}</span>
                  </td>
                </tr>
              ))}
            </tbody>
