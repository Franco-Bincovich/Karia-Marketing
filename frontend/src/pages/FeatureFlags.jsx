...
export default function FeatureFlags() {
  ...
  useEffect(() => {
    if (!clienteId) return;
    get(ENDPOINTS.FEATURE_FLAGS, { cliente_id: clienteId })
  ...
  async function toggleFlag(flag) {
    try {
      const { data } = await patch(
        `${ENDPOINTS.FEATURE_FLAG(flag.feature)}?cliente_id=${clienteId}`,
        { habilitado: !flag.habilitado, modo: flag.modo }
      );
  ...
  async function changeModo(flag, modo) {
    try {
      const { data } = await patch(
        `${ENDPOINTS.FEATURE_FLAG(flag.feature)}?cliente_id=${clienteId}`,
        { habilitado: flag.habilitado, modo }
      );
      setFlags((prev) => prev.map((f) => (f.id === flag.id ? data : f)));
    } catch {}
  }
    ...
    <Layout title="Feature Flags">
      {!flags.length ? (
        <EmptyState
          ...
          description="No hay feature flags configurados para este cliente"
        />
      ) : (
        <div
          style={{
        ...
        >
          {flags.map((flag) => (
            <div key={flag.id} style={card}>
              ...
              >
                {["ia", "manual", "autopilot", "copilot"].map((m) => (
