...
function JsonField({ data }) {
  ...
  if (typeof data === "string")
    return <span style={{ color: "var(--text)", fontSize: 14 }}>{data}</span>;
  if (data.respuesta)
    return <span style={{ color: "var(--text)", fontSize: 14 }}>{data.respuesta}</span>;
  return (
...
export default function PerfilMarca() {
  ...
  useEffect(() => {
    get(ENDPOINTS.MARCA_PERFIL)
      .then((r) => setPerfil(r.data))
      .catch(() => {})
      .finally(() => setLoading(false));
    get(ENDPOINTS.MARCA_DOCUMENTOS)
  ...
  if (!perfil)
    return (
      <Layout title="Perfil de Marca">
        <p style={{ color: "var(--text-muted)" }}>No se pudo cargar el perfil.</p>
      </Layout>
    );
  ...
  return (
    <Layout title="Perfil de Marca">
      <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 16 }}>
        <button style={btnPrimary} onClick={() => navigate("/onboarding")}>
          Editar Onboarding
        </button>
      </div>
      ...
      <div style={card}>
        ...
        <div style={fieldWrap}>
          ...
          <ColorSwatches items={perfil.colores_marca} />
        </div>
        ...
        <div style={fieldWrap}>
          ...
          {perfil.palabras_prohibidas?.length > 0 ? (
            <div>
              {perfil.palabras_prohibidas.map((t, i) => (
                <span
                  key={i}
                  style={{ ...tag, background: "var(--danger-bg)", color: "var(--danger-text)" }}
                >
                  {t}
                </span>
              ))}
      ...
      <div style={card}>
        ...
        <p style={{ fontSize: 12, color: "var(--text-muted)", marginBottom: 12 }}>
          Subí briefs, manuales de marca o cualquier documento para dar más contexto a los agentes
          de IA.
        </p>
        ...
        {docError && (
          <div className="msg-error" style={{ marginBottom: 12, borderRadius: 8 }}>
            ...
            <button className="msg-dismiss" onClick={() => setDocError("")}>
              ×
            </button>
          </div>
        )}
        ...
        {docs.length === 0 ? (
          <EmptyState
            ...
            description="Subí tu primer documento de marca"
          />
        ) : (
          <div>
            {docs.map((d) => (
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: 11, color: "var(--text-muted)" }}>
                      {d.tipo.toUpperCase()} —{" "}
                      {d.chars_extraidos > 0
                        ? `${d.chars_extraidos.toLocaleString()} chars extraídos`
                        : "Sin texto extraído"}{" "}
                      — {formatDate(d.created_at)}
                    </div>
                  </div>
                </div>
                <button style={btnDanger} onClick={() => handleDelete(d.id)}>
                  Eliminar
                </button>
              </div>
            ))}
      ...
      <div
        style={{ textAlign: "center", padding: "8px 0", color: "var(--text-muted)", fontSize: 12 }}
      >
        Plan: {perfil.plan} — Última actualización:{" "}
        {perfil.updated_at ? new Date(perfil.updated_at).toLocaleDateString("es-AR") : "—"}
      </div>
    </Layout>
  );
}
