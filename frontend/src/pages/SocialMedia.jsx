...
const th          = { textAlign: "left", fontSize: 10, color: "var(--text-muted)", textTransform: "uppercase", padding: "8px 12px", borderBottom: "1px solid var(--border)", fontWeight: 600, letterSpacing: "0.06em" };
...
export default function SocialMedia() {
  ...
  async function conectarCuenta() {
    ...
    try {
      ...
      const { data } = await post(ENDPOINTS.SOCIAL_CONECTAR, { platform: connectPlatform, callback_url: callbackUrl });
    ...
    <Layout title="Redes Sociales">
      ...
      <div style={card}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
          <h3 style={{ fontSize: 15, fontWeight: 700, color: "var(--text)" }}>Cuentas Conectadas</h3>
        </div>
        {cuentas.filter(c => c.activa).length === 0 ? (
          <EmptyState icon="📱" title="Sin cuentas conectadas" description="Conectá tu primera red social abajo" />
        ) : (
          <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
            {cuentas.filter(c => c.activa).map(c => (
              <div key={c.id} style={{ border: "1px solid var(--border)", borderRadius: 10, padding: "12px 16px", display: "flex", alignItems: "center", gap: 10, background: "var(--surface-2)" }}>
                <span style={{ fontSize: 20 }}>{redIcons[c.red_social] || "🌐"}</span>
                <div>
                  <div style={{ fontSize: 13, fontWeight: 600, color: "var(--text)" }}>{c.nombre_cuenta}</div>
                  <div style={{ fontSize: 11, color: "var(--text-muted)" }}>{c.red_social}</div>
                </div>
                <span style={{ background: "var(--success-bg)", color: "var(--success-text)", padding: "2px 8px", borderRadius: 6, fontSize: 10, fontWeight: 600 }}>Activa</span>
                <button
      ...
      {/* Conectar nueva */}
      <div style={card}>
        <h3 style={{ fontSize: 15, fontWeight: 700, marginBottom: 16, color: "var(--text)" }}>Conectar Cuenta</h3>
      ...
      <div style={{ ...card, overflowX: "auto" }}>
        <h3 style={{ fontSize: 15, fontWeight: 700, marginBottom: 12, color: "var(--text)" }}>Historial de Publicaciones ({pubs.length})</h3>
        <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 700 }}>
          ...
          <tbody>
            {pubs.length === 0 && (
              <tr><td colSpan={6} style={{ ...td, textAlign: "center", color: "var(--text-muted)" }}>Sin publicaciones</td></tr>
            )}
            {pubs.map(p => (
              <tr key={p.id}>
                <td style={td}><span style={{ fontSize: 16, marginRight: 4 }}>{redIcons[p.red_social] || "🌐"}</span>{p.red_social}</td>
                <td style={{ ...td, maxWidth: 280, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{p.copy_publicado || "—"}</td>
                <td style={td}><span style={estadoBadge(p.estado)}>{p.estado}</span></td>
                <td style={td}>{p.estado === "programado" ? formatDate(p.programado_para) : formatDate(p.publicado_at)}</td>
