...
const estadoLabel = (estado) =>
  ({ activo: "Activo", por_vencer: "Por vencer", vencido: "Vencido" })[estado] || "Vencido";
...
export default function Clientes() {
  ...
  async function handleSubmit(e) {
    e.preventDefault();
    if (!form.nombre || !form.email_admin) {
  ...
  async function toggleEstado(cliente) {
    ...
    try {
      const { data } = await patch(ENDPOINTS.CLIENTE_ESTADO(cliente.id), {
    ...
    <Layout title="Clientes">
      ...
      {createdCliente && !showForm && (
        <div style={{ ...card, borderColor: "var(--success)", background: "var(--success-bg)" }}>
          <div
            style={{ fontSize: 14, fontWeight: 600, color: "var(--success-text)", marginBottom: 8 }}
          >
          ...
          <div style={{ fontSize: 13, color: "var(--text-secondary)", marginTop: 4 }}>
            Plan: <strong>{createdCliente.plan}</strong> · Vencimiento:{" "}
            <strong>{formatDate(createdCliente.fecha_vencimiento)}</strong> (
            {createdCliente.dias_restantes} días)
          </div>
          <button
      ...
      {showForm && (
        <form style={card} onSubmit={handleSubmit}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            ...
            <button style={btnSecondary} type="button" onClick={() => setShowForm(false)}>
              Cancelar
            </button>
          </div>
        </form>
          ...
          <tbody>
            ...
            {clientes.map((c) => (
              <tr key={c.id}>
                ...
                <td style={td}>
                  <span style={planBadge(c.plan)}>{c.plan}</span>
                </td>
                <td style={td}>
                  <span style={estadoBadge(c.estado_vencimiento)}>
                    {estadoLabel(c.estado_vencimiento)}
                  </span>
                </td>
                ...
                <td
                  style={{
                    ...td,
                    fontWeight: 600,
                    color: c.dias_restantes <= 7 ? "var(--warning-text)" : "var(--text-secondary)",
                  }}
                >
                  {c.dias_restantes}d
                </td>
