...
const STATUS_COLORS = { disponible: "var(--success)", bloqueado_v1: "var(--text-muted)", solo_premium: "var(--purple-text)" };
const STATUS_LABELS = { bloqueado_v1: "Próximamente", solo_premium: "Premium" };

function HoverTooltip({ node }) {
  const deps = node.dependencias?.length > 0 ? ` — Depende de: ${node.dependencias.join(", ")}` : "";
  return (
    <div style={{ marginTop: 16, padding: "12px 16px", background: "var(--surface)", border: `1px solid ${node.area_color}`, borderRadius: 10, maxWidth: 420, width: "100%" }}>
      <div style={{ fontSize: 14, fontWeight: 700, color: "var(--text)", marginBottom: 4 }}>{node.icon} {node.label}</div>
      <div style={{ fontSize: 12, color: "var(--text-secondary)", marginBottom: 4 }}>{node.rol}</div>
      <div style={{ fontSize: 11, color: "var(--text-muted)" }}>Área: {node.area} — Reporta a: {node.reporta_a || "—"}{deps}</div>
    </div>
  );
}
...
function AgentCard({ node, onHover }) {
  ...
  return (
    <div
      ...
      style={{
        minWidth: 180, borderRadius: 10, border: `2px solid ${border}`,
    ...
    >
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
        {node.estado === "disponible" ? (
          <span style={{ fontSize: 11, fontWeight: 600, color: active ? "var(--success-text)" : "var(--text-muted)" }}>
            {active ? "Activo" : "Inactivo"}
          </span>
        ) : (
          <span style={{ fontSize: 11, fontWeight: 600, color: STATUS_COLORS[node.estado] || "var(--text-muted)" }}>
...
function CollabCard({ node, onHover }) {
  return (
    <div onMouseEnter={() => onHover(node)} onMouseLeave={() => onHover(null)}
      style={{ minWidth: 140, borderRadius: 8, border: `1.5px dashed ${node.area_color}60`, background: "var(--surface)", padding: "6px 10px", cursor: "default" }}>
      <div style={{ fontSize: 9, fontWeight: 600, color: "var(--text-muted)", textTransform: "uppercase", letterSpacing: 0.5, marginBottom: 2 }}>Colaborador</div>
      <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", marginTop: 4 }}>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 6, justifyContent: "center" }}>
        {collabs.map(c => <CollabCard key={c.nombre} node={c} onHover={onHover} />)}
      </div>
    </div>
  );
}
...
function AreaColumn({ name, color, agents, collabsByParent, onHover }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 10, flex: "1 1 180px", minWidth: 180 }}>
      <div style={{ width: 10, height: 10, borderRadius: "50%", background: color, flexShrink: 0 }} />
      <div style={{
        fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: 1,
        color, textAlign: "center", padding: "4px 12px",
        border: `1px solid ${color}40`, borderRadius: 6, background: `${color}10`,
      }}>
        {name}
      </div>
      {agents.map(a => (
        <div key={a.nombre} style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
...
export default function OrgChart({ nodos, areas }) {
    ...
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 0 }}>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", justifyContent: "center", gap: 20, width: "100%" }}>
        {areaOrder.map(([name, info]) => {
          ...
          return (
            <AreaColumn key={name} name={name} color={info.color}
              agents={agents} collabsByParent={collabsByParent} onHover={setHovered} />
          );
        })}
      </div>
