            ...
            return (
              <button
                ...
                style={{
                  ...
                  cursor: "pointer",
                  background: active
                    ? m === "autopilot" ? "var(--success-bg)" : "var(--purple-bg)"
                    : "transparent",
                  color: active
                    ? m === "autopilot" ? "var(--success-text)" : "var(--purple-text)"
                    : "var(--text-muted)",
          ...
          {dropdownOpen && (
            <div style={{
              ...
              <div style={{ padding: "10px 12px 8px", borderBottom: "1px solid var(--border-subtle)", marginBottom: 4 }}>
                <div style={{ fontSize: 13, fontWeight: 700, color: "var(--text)" }}>{user?.nombre || "Usuario"}</div>
                <div style={{ fontSize: 11, color: "var(--text-muted)", marginTop: 2 }}>{user?.email || user?.rol}</div>
              </div>

              {[
                { label: "Ver perfil de marca", icon: "◫", action: () => { navigate("/marca/perfil"); setDropdownOpen(false); } },
                { label: "Configuración",        icon: "⊙", action: () => { navigate("/onboarding"); setDropdownOpen(false); } },
                { label: "Cambiar marca",         icon: "↔", action: () => { navigate("/marcas"); setDropdownOpen(false); }, show: user?.rol !== "admin_cliente" },
