...
export default function Sidebar({ collapsed, onToggle, onNavigate, isMobile }) {
  ...
  useEffect(() => {
    if (marcaActiva) {
      api.get("/api/comunidad/mensajes/pendientes")
  ...
  return (
    <aside style={{
        ...
        <div style={{
          ...
          {!isMobile && (
            <button onClick={onToggle} style={{
              ...
              width: 28, height: 18, cursor: "pointer", color: "var(--sidebar-text)",
        ...
        <div style={{
          ...
          {!isMobile && (
            <button onClick={onToggle} style={{
              ...
              padding: "7px 8px", cursor: "pointer", color: "var(--sidebar-text)",
              fontSize: 13, lineHeight: 1, display: "flex", alignItems: "center", justifyContent: "center",
              transition: "background var(--t-fast)", flexShrink: 0,
            }}
      ...
      {!collapsed && marcas.length > 0 && (
        <div style={{ padding: "12px 16px", borderBottom: "1px solid var(--sidebar-border)" }}>
          {marcas.length === 1 ? (
            ...
            }} value={marcaActiva?.id || ""} onChange={handleMarcaChange}>
      ...
      <nav style={{
        ...
        padding: collapsed ? "6px 0" : "8px 8px", minHeight: 0,
      }}>
        {visibleSections.map((sec, secIdx) => (
          <div key={sec.title}>
            {collapsed ? (
              secIdx > 0 && <div style={{ height: 1, background: "rgba(255,255,255,0.07)", margin: "6px 12px" }} />
            ) : (
              <div onClick={() => toggleSection(sec.title)} style={{
                fontSize: 9, fontWeight: 700, color: "var(--sidebar-section)", textTransform: "uppercase",
                letterSpacing: "0.08em", padding: "14px 8px 6px", cursor: "pointer",
                display: "flex", alignItems: "center", justifyContent: "space-between", userSelect: "none",
              }}>
                <span>{sec.title}</span>
                <span style={{ fontSize: 9, transition: "transform var(--t-fast)", transform: openSections[sec.title] ? "rotate(0)" : "rotate(-90deg)", color: "var(--sidebar-text)" }}>▾</span>
              </div>
            )}
            ...
            {(collapsed || openSections[sec.title]) && sec.items.map(item => {
              ...
              if (collapsed) {
                return (
                  <Tooltip key={item.path} text={item.v2 ? `${item.label} (V2)` : item.label} delay={250} placement="right">
                    <Link to={disabled ? "#" : item.path} onClick={disabled ? (e => e.preventDefault()) : handleLinkClick} style={{
                      display: "flex", alignItems: "center", justifyContent: "center",
                      width: 40, height: 40, margin: "2px auto", borderRadius: 10, fontSize: 20,
                      color: active ? "#fff" : "var(--sidebar-text)",
                      background: active ? "var(--sidebar-active)" : "transparent",
                      border: active ? "1px solid rgba(255,107,43,0.35)" : "1px solid transparent",
                      cursor: disabled ? "default" : "pointer", transition: "all var(--t-fast)", textDecoration: "none",
              ...
              return (
                <Link key={item.path} to={disabled ? "#" : item.path} onClick={disabled ? (e => e.preventDefault()) : handleLinkClick} style={{
                  display: "flex", alignItems: "center", gap: 10, padding: "9px 10px",
                  borderRadius: 8, fontSize: 13, fontWeight: active ? 600 : 400,
                  color: active ? "var(--sidebar-text-active)" : "var(--sidebar-text)",
                  background: active ? "var(--sidebar-active)" : "transparent",
                  cursor: disabled ? "default" : "pointer", transition: "all var(--t-fast)", marginBottom: 2,
                  whiteSpace: "nowrap", overflow: "hidden", textDecoration: "none", position: "relative",
                  opacity: disabled ? 0.4 : 1, pointerEvents: disabled ? "none" : "auto",
                }}
                  onMouseEnter={e => { if (!active) { e.currentTarget.style.background = "var(--sidebar-hover)"; e.currentTarget.style.color = "#fff"; } }}
                  onMouseLeave={e => { if (!active) { e.currentTarget.style.background = "transparent"; e.currentTarget.style.color = "var(--sidebar-text)"; } }}
                >
                  {active && (
                    <span style={{
                      ...
                      width: 3, height: 16, background: "var(--primary)", borderRadius: "0 2px 2px 0",
                    }} />
                  )}
                  ...
                  {item.badge && pendientesCount > 0 && (
                    <span style={{
                      ...
                      padding: "2px 5px", borderRadius: 9, minWidth: 16, textAlign: "center",
        ...
        {collapsed ? (
          <Tooltip text={user?.nombre || "Usuario"} delay={200} placement="right">
            <div style={{
              ...
              display: "flex", alignItems: "center", justifyContent: "center",
              color: "#fff", fontSize: 13, fontWeight: 700, cursor: "default", flexShrink: 0,
            }}>{initials}</div>
          </Tooltip>
        ) : (
          <div style={{ display: "flex", alignItems: "center", gap: 10, width: "100%" }}>
            ...
            <div style={{ minWidth: 0, flex: 1 }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 5, marginTop: 2 }}>
                {user?.rol === "superadmin" ? (
                  <span style={{ fontSize: 9, fontWeight: 700, color: "#F5A623", background: "rgba(245,166,35,0.18)", padding: "1px 6px", borderRadius: 4, letterSpacing: "0.06em" }}>SUPERADMIN</span>
                ) : (
                  <span style={{ fontSize: 9, fontWeight: 700, color: plan === "Premium" ? "#A78BFA" : "rgba(255,255,255,0.60)", background: plan === "Premium" ? "rgba(139,92,246,0.2)" : "rgba(255,255,255,0.08)", padding: "1px 6px", borderRadius: 4, letterSpacing: "0.06em" }}>{plan?.toUpperCase()}</span>
                )}
              </div>
            </div>
            <button onClick={handleLogout} style={{
              background: "rgba(255,255,255,0.06)", border: "none", borderRadius: 8,
              padding: "7px 8px", color: "rgba(255,255,255,0.65)", fontSize: 11,
              cursor: "pointer", flexShrink: 0, transition: "background var(--t-fast)",
            }}
